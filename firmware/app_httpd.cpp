...
esp_err_t snaplens_handler(httpd_req_t *req) {
...
esp_http_client_config_t config = {};
config.url = "https://snap-lens.onrender.com/evaluate";
config.method = HTTP_METHOD_POST;
config.timeout_ms = 10000;
config.cert_pem = NULL;
config.skip_cert_common_name_check = true;
  ...
  int status = -1;
  char response_buffer[256] = {0}; // change size if needed
  ...
  if (err == ESP_OK) {
    ...
    if (content_length < sizeof(response_buffer)) {
      ...
      Serial.printf("SnapLens Response (%d): %s\n", status, response_buffer);
    } else {
      Serial.println("SnapLens response too large to store");
    }
  } else {
    Serial.printf("SnapLens POST failed: %s\n", esp_err_to_name(err));
  }

  if (err == ESP_OK) {
    status = esp_http_client_get_status_code(client);
    Serial.printf("SnapLens POST status = %d\n", status);
  } else {
    Serial.printf("SnapLens POST failed: %s\n", esp_err_to_name(err));
  }
  ...
  if (status == 200) {
    httpd_resp_send(req, "SnapLens success", HTTPD_RESP_USE_STRLEN);
  } else {
    httpd_resp_send(req, "SnapLens failed", HTTPD_RESP_USE_STRLEN);
  }
...
void startCameraServer() {
  ...
  httpd_uri_t snaplens_uri = {
    .uri       = "/snapLens",
    .method    = HTTP_POST,
    .handler   = snaplens_handler,
    .user_ctx  = NULL
};

httpd_register_uri_handler(stream_httpd, &snaplens_uri);
